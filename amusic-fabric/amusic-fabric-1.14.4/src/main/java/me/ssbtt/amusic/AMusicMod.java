...
/**
 * Fabric 客户端 Mod 主入口（1.14.4 旧版网络 API）。
 *
 * <p>负责注册：插件通道（S2C/C2S）、按键绑定、客户端 Tick、HUD 渲染等。</p>
 *
 * <p>注意：1.14.4 使用旧版 Fabric 网络 API（{@code Identifier} + {@code PacketByteBuf}），
 * 不支持 {@code ClientReceiveMessageEvents}（1.19+ 才有），无 {@code MatrixStack} HUD 回调。</p>
 *
 * @author 真心
...
public class AMusicMod implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        // 注意：1.14.4 没有 ClientReceiveMessageEvents，聊天链接捕获功能不可用
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            if (client.player == null || client.world == null) {
                return;
            }
            handleKey(AMusicKeys.OPEN_SETTINGS, () -> client.openScreen(SettingsScreen.build(client.currentScreen)));
            handleKey(AMusicKeys.HISTORY, () -> client.openScreen(new HistoryScreen()));
            handleKey(AMusicKeys.PLAYLIST, () -> client.openScreen(new PlaylistScreen()));
            ...
            handleKey(AMusicKeys.VOLUME_POSITION, () -> client.openScreen(new VolumeOverlayPositionScreen()));
        ...
        // 注册 HUD 渲染：音量条 overlay（1.14.4 无 MatrixStack，回调仅接收 tickDelta）
        HudRenderCallback.EVENT.register(tickDelta -> {
            MinecraftClient mc = MinecraftClient.getInstance();
            if (mc.player != null) {
                volumeOverlay.render();
            }
        });
    }
