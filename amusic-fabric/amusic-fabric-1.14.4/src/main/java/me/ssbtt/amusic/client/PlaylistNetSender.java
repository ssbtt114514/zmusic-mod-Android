...
/**
 * 歌单网络请求发送器（Fabric 版本，旧版网络 API）。
 *
 * <p>通过 AMusic 插件通道（{@code amusic:channel}）向服务端发送歌单相关请求。
 * 使用旧版 Fabric 网络 API（{@code Identifier} + {@code PacketByteBuf}），
 * 适用于 1.20.1/1.20.4 等不支持 {@code PayloadTypeRegistry} 的版本。</p>
 *
 * <p>数据包格式（客户端→服务端）：</p>
 * <ul>
