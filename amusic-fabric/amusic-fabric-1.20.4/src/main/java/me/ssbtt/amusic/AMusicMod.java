...
/**
 * Fabric 客户端 Mod 主入口（1.20.1 / 1.20.4 旧版网络 API）。
 *
 * <p>负责注册：插件通道（S2C/C2S）、按键绑定、聊天事件、客户端 Tick、HUD 渲染等。</p>
 *
 * <p>注意：1.20.1/1.20.4 使用旧版 Fabric 网络 API（{@code Identifier} + {@code PacketByteBuf}），
 * 不支持 1.20.6+ 的 {@code PayloadTypeRegistry} 和 {@code CustomPayload}。</p>
 *
 * @author 真心
