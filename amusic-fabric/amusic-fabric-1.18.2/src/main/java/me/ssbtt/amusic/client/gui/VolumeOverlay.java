...
/**
 * 音量调节 overlay（Fabric 1.18-1.19 版本，使用 MatrixStack）。
 *
 * <p>调节音量时在屏幕上立即显示音量条，2 秒后消失，无淡出。
 * 由 {@code HudRenderCallback} 在 HUD 渲染时调用 {@link #render(MatrixStack)}。</p>
 *
 * @author ssbtt
 * @since 2026-07-28
 */
...
public class VolumeOverlay {
    ...
    /**
     * 渲染音量条（由 HUD 回调每帧调用，1.18-1.19 使用 MatrixStack）。
     */
    public void render(MatrixStack matrices) {
        ...
        DrawableHelper.fill(matrices, x - 4, y - 4, x + barWidth + 4, y + barHeight + 4, 0xB0000000);

        int barY = y + 2;
        int barFilled = (int) ((barWidth - 4) * displayedVolume);
        DrawableHelper.fill(matrices, x + 2, barY, x + barWidth - 2, barY + barHeight - 4, 0xFF333333);
        if (displayedMuted) {
            DrawableHelper.fill(matrices, x + 2, barY, x + 2 + barFilled, barY + barHeight - 4, 0xFFFF5555);
        } else {
            DrawableHelper.fill(matrices, x + 2, barY, x + 2 + barFilled, barY + barHeight - 4, 0xFF55FF55);
        }

        int pct = (int) (displayedVolume * 100);
        String text = displayedMuted ? "已静音" : "音量 " + pct + "%";
        DrawableHelper.drawCenteredTextWithShadow(matrices, mc.textRenderer, text, screenWidth / 2, y + barHeight - 12, 0xFFFFFFFF);
    }
}
