...
/**
 * Fabric 客户端 Mod 主入口（1.19.4 旧版网络 API + MatrixStack GUI）。
 *
 * <p>负责注册：插件通道（S2C/C2S）、按键绑定、聊天事件、客户端 Tick、HUD 渲染等。</p>
 *
 * <p>注意：1.19.4 使用旧版 Fabric 网络 API（{@code Identifier} + {@code PacketByteBuf}），
 * 支持 {@code ClientReceiveMessageEvents}，GUI 渲染使用 {@code MatrixStack}（1.20+ 才有 {@code DrawContext}）。</p>
 *
 * @author 真心
...
public class AMusicMod implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        // 注册 HUD 渲染：音量条 overlay（1.19 使用 MatrixStack，1.20+ 才有 DrawContext）
