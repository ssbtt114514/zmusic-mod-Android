...
/**
 * 歌单界面（Forge 1.19.4 版本，使用 PoseStack 渲染）。
 *
 * @author 真心
 * @since 2026-07-25
 */
...
public class PlaylistScreen extends Screen {
    ...
    protected void init() {
        ...
        addRenderableWidget(Button.builder(Component.literal("创建"), b -> createPlaylist()).bounds(10 + halfW - 70, this.height - 50, 60, 18).build());

        addRenderableWidget(Button.builder(Component.literal("播放全部"), b -> playAll()).bounds(20 + halfW, this.height - 50, 80, 18).build());

        addRenderableWidget(Button.builder(Component.literal("添加当前歌曲"), b -> addCurrentSong()).bounds(20 + halfW + 90, this.height - 50, 90, 18).build());

        addRenderableWidget(Button.builder(Component.literal("停止播放"), b -> stopPlaylist()).bounds(20 + halfW + 190, this.height - 50, 80, 18).build());

        addRenderableWidget(Button.builder(Component.literal("在线歌单"), b -> openOnlinePlaylists()).bounds(20 + halfW + 280, this.height - 50, 80, 18).build());

        addRenderableWidget(Button.builder(Component.translatable("gui.done"), b -> onClose()).bounds(this.width / 2 - 50, this.height - 24, 100, 18).build());
    }
