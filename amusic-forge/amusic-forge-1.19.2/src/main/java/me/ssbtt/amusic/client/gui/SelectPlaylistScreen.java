...
/**
 * 歌单选择界面（Forge 1.19.2 版本，使用 PoseStack 渲染）。
 *
 * @author ssbtt
 * @since 2026-07-27
 */
...
public class SelectPlaylistScreen extends Screen {
    ...
    protected void init() {
        ...
        addRenderableWidget(new Button(165, this.height - 50, 90, 18, Component.literal("新建并添加"), b -> createAndAdd()));

        addRenderableWidget(new Button(this.width / 2 - 100, this.height - 24, 200, 20, Component.translatable("gui.cancel"), b -> onClose()));
    }
    ...
    public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
        ...
        this.font.draw(poseStack, "选择要将「" + (entry != null ? entry.getName() : "") + "」添加到的歌单",
                10, 24, 0xAAAAFF);
        list.render(poseStack, mouseX, mouseY, partialTick);

        if (!statusMessage.isEmpty()) {
            GuiComponent.drawCenteredString(poseStack, this.font, statusMessage, this.width / 2, this.height - 62, statusColor);
        }
    }
    ...
    private class PlaylistList extends ObjectSelectionList<PlaylistList.Entry> {
        ...
        private class Entry extends ObjectSelectionList.Entry<Entry> {
            ...
            public void render(PoseStack poseStack, int index, int top, int left, int width, int height, int mouseX, int mouseY, boolean hovering, float partialTick) {
                ...
                font.draw(poseStack, name + countStr, left + 4, top + 4, 0xFFFFFF);
                ...
                font.draw(poseStack, label, addBtnX, top + 4, hover ? 0xFFFF55 : 0x55FF55);
            }
