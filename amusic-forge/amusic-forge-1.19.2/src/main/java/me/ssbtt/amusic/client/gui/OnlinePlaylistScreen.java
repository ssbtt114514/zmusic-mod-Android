...
/**
 * 在线歌单界面（Forge 1.19.2 版本，使用 PoseStack 渲染）。
 *
 * @author ssbtt
 * @since 2026-07-27
 */
...
public class OnlinePlaylistScreen extends Screen {
    ...
    protected void init() {
        ...
        addRenderableWidget(new Button(width / 2 - 155, 5, 70, 18, Component.literal("公开歌单"), b -> switchMode(Mode.PUBLIC)));
        addRenderableWidget(new Button(width / 2 - 80, 5, 70, 18, Component.literal("我的歌单"), b -> switchMode(Mode.MINE)));
        addRenderableWidget(new Button(width / 2 - 5, 5, 70, 18, Component.literal("上传歌单"), b -> openUploadDialog()));
        addRenderableWidget(new Button(width / 2 + 70, 5, 70, 18, Component.literal("刷新"), b -> refreshList()));

        addRenderableWidget(new Button(width / 2 - 40, height - 25, 80, 18, Component.literal("返回"), b -> onClose()));
    ...
    public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
        ...
        GuiComponent.drawCenteredString(poseStack, this.font, this.title, this.width / 2, 27, 0xFFFFFF);
        list.render(poseStack, mouseX, mouseY, partialTick);
        if (!statusMessage.isEmpty()) {
            GuiComponent.drawCenteredString(poseStack, this.font, statusMessage, this.width / 2, height - 45, statusColor);
        }
    }
    ...
    private class OnlineList extends ObjectSelectionList<OnlineList.Entry> {
        ...
        private class Entry extends ObjectSelectionList.Entry<Entry> {
            ...
            public void render(PoseStack poseStack, int index, int top, int left, int width, int height, int mouseX, int mouseY, boolean hovering, float partialTick) {
                ...
                font.draw(poseStack, info.name, left + 2, top + 2, 0xFFFFFF);
                String meta = "作者: " + (info.author != null ? info.author : "?") + " | " + info.songCount + "首";
                font.draw(poseStack, meta, left + 2, top + 11, 0xAAAAFF);
                ...
                font.draw(poseStack, dlLabel, downloadBtnX, top + 7, hoverDl ? 0xFFFF55 : 0x55FF55);
            }
    ...
    public static class UploadPlaylistScreen extends Screen {
        ...
        protected void init() {
            ...
            addRenderableWidget(new Button(width / 2 - 40, height - 25, 80, 18, Component.literal("返回"), b -> {
        ...
        public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
            renderBackground(poseStack);
            super.render(poseStack, mouseX, mouseY, partialTick);
            GuiComponent.drawCenteredString(poseStack, this.font, "选择要上传的歌单（点击歌单名上传）", this.width / 2, 10, 0xFFFFFF);
            list.render(poseStack, mouseX, mouseY, partialTick);
        }
        ...
        private class UploadList extends ObjectSelectionList<UploadList.Entry> {
            ...
            private class Entry extends ObjectSelectionList.Entry<Entry> {
                ...
                public void render(PoseStack poseStack, int index, int top, int left, int width, int height, int mouseX, int mouseY, boolean hovering, float partialTick) {
                    ...
                    font.draw(poseStack, name + " (" + count + "首)", left + 2, top + 2, 0xFFFFFF);
                    ...
                    font.draw(poseStack, pubLabel, uploadBtnX, top + 11, hoverPub ? 0xFFFF55 : 0x55FF55);
                    ...
                    font.draw(poseStack, privLabel, uploadPrivateBtnX, top + 11, hoverPriv ? 0xFFFF55 : 0xFFAA00);
                }
