...
/**
 * 音量条位置调整界面（Forge 1.19.2 版本，使用 PoseStack 渲染）。
 *
 * @author ssbtt
 * @since 2026-07-25
 */
...
public class VolumeOverlayPositionScreen extends Screen {
    ...
    protected void init() {
        ...
        addRenderableWidget(new Button(10, 10, 100, 20, Component.literal("重置到默认"), b -> {
            barX = (this.width - BAR_WIDTH) / 2;
            barY = this.height - 40;
        }));

        addRenderableWidget(new Button(this.width / 2 - 110, this.height - 28, 100, 20, Component.literal("保存"), b -> save()));

        addRenderableWidget(new Button(this.width / 2 + 10, this.height - 28, 100, 20, Component.translatable("gui.cancel"), b -> onClose()));
    }

    @Override
    public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
        super.render(poseStack, mouseX, mouseY, partialTick);
        GuiComponent.drawCenteredString(poseStack, this.font, this.title, this.width / 2, 14, 0xFFFFFF);
        GuiComponent.drawCenteredString(poseStack, this.font, "拖动绿色条到目标位置，点击「保存」", this.width / 2, 26, 0xAAAAFF);

        int barY2 = barY + 2;
        int barFilled = (int) ((BAR_WIDTH - 4) * 0.7);
        GuiComponent.fill(poseStack, barX - 4, barY - 4, barX + BAR_WIDTH + 4, barY + BAR_HEIGHT + 4, 0xB0000000);
        GuiComponent.fill(poseStack, barX + 2, barY2, barX + BAR_WIDTH - 2, barY2 + BAR_HEIGHT - 4, 0xFF333333);
        GuiComponent.fill(poseStack, barX + 2, barY2, barX + 2 + barFilled, barY2 + BAR_HEIGHT - 4, 0xFF55FF55);
        GuiComponent.drawCenteredString(poseStack, this.font, "音量 70% (可拖动)", this.width / 2, barY + BAR_HEIGHT - 12, 0xFFFFFFFF);
    }
