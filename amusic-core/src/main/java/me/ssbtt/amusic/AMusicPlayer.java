...
public class AMusicPlayer {
    ...
    private final ExecutorService commandExecutor = Executors.newSingleThreadExecutor(new ThreadFactory() {
        ...
        public Thread newThread(Runnable runnable) {
            Thread thread = new Thread(runnable, "amusic-command");
    ...
    public AMusicPlayer() {
        log.info("AMusicPlayer constructor start");
        backend = createBackend();
        log.info("AMusic player initialized (pure Java, backend={})", backend.name());
    }

    // ---- 公开接口（与原 native 版本完全一致）----

    public int play(String url) {
        log.info("AMusicPlayer.play() called: url={}, thread={}", url, Thread.currentThread().getName());
        doPlay(url);
        log.info("AMusicPlayer.play() returned for url={}", url);
        return 0;
    }

    public void playAsync(final String url) {
        log.info("AMusicPlayer.playAsync() called: url={}, thread={}", url, Thread.currentThread().getName());
        executeCommand(new Runnable() {
            @Override
            public void run() {
                if (!running) {
                    log.warn("AMusicPlayer.playAsync: player not running, abort");
                    return;
                }
                log.info("AMusicPlayer.playAsync: doStop before new play");
                doStop();
                log.info("AMusicPlayer.playAsync: doPlay with url={}", url);
                doPlay(url);
            }
        });
    }
    ...
    public int pause() {
        log.info("AMusicPlayer.pause() called, current state={}", state);
        backend.pause();
    ...
    public int stop() {
        log.info("AMusicPlayer.stop() called, current state={}", state);
        doStop();
        return 0;
    }

    public void stopAsync() {
        log.info("AMusicPlayer.stopAsync() called, current state={}", state);
        executeCommand(new Runnable() {
            @Override
            public void run() {
                doStop();
    ...
    public int resume() {
        log.info("AMusicPlayer.resume() called, current state={}", state);
        backend.resume();
    ...
    public int seek(long positionMs) {
        log.info("AMusicPlayer.seek() called: pos={}ms, current state={}", positionMs, state);
    ...
    public int setVolume(final float volume) {
        ...
        log.info("AMusic volume change: {}", volume);
        executeCommand(new Runnable() {
            @Override
            public void run() {
                if (!running) return;
    ...
    public synchronized void destroy() {
        log.info("AMusicPlayer.destroy() called, backend={}", backend == null ? "null" : backend.name());
        running = false;
        commandExecutor.shutdownNow();
        synchronized (playLock) {
            try {
                backend.destroy();
                log.info("AMusicPlayer.destroy: backend.destroy() returned");
            } catch (Throwable t) {
                log.warn("AMusicPlayer.destroy: backend.destroy() threw: {}", t.getMessage(), t);
            }
        }
        log.info("AMusicPlayer.destroy end");
    }
    ...
    private void doPlay(String url) {
        if (url == null || url.trim().isEmpty()) {
            log.warn("AMusic play called with empty URL");
            return;
        }
        ...
        synchronized (playLock) {
            ...
            try {
                ...
                setState(STATE_PLAYING);
                log.info("AMusic playback started: {}", url);
            } catch (Exception e) {
                log.error("AMusic playback failed: {} - {}", url, e.getMessage(), e);
    ...
    private void doStop() {
        ...
        synchronized (playLock) {
            ...
            try {
                ...
                log.warn("AMusic stop error: {}", e.getMessage());
            }
    ...
    private class JLayerBackend implements AudioBackend {
        ...
        public void play(String url) throws Exception {
            ...
            connection.setRequestProperty("User-Agent", "AMusic/3.7");
            ...
            me.ssbtt.amusic.config.AMusicConfig cfg = AMusic.getConfig();
            if (cfg != null) {
